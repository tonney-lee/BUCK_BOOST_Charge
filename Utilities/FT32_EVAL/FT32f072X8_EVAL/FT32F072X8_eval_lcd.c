/**
  ******************************************************************************
  * @file    FT32F072X8_eval_lcd.c
  * @author  MCD Application Team
  * @version V1.0.0
  * @date    16-January-2014
  * @brief   This file includes the LCD driver for AM240320LGTNQW00H (HX8347-D) 
  *          Liquid Crystal Display Module of FT32F072X8-EVAL board.
  ******************************************************************************
  * @attention
  *
...
/** @defgroup FT32F072X8_EVAL_LCD_Private_Functions
  * @{
  */ 
...
void LCD_WriteRegIndex(uint8_t LCD_Reg)
{
  ...
  FT_SPI_WriteRead(0x00);
  
  FT_SPI_WriteRead(LCD_Reg);
...
void LCD_WriteReg(uint8_t LCD_Reg, uint16_t LCD_RegValue)
{
  ...
  /* Reset LCD control line(/CS) and Send Start-Byte */
  LCD_nCS_StartByte(START_BYTE | LCD_WRITE_REG);
  FT_SPI_WriteRead(LCD_RegValue>>8);
  FT_SPI_WriteRead(LCD_RegValue & 0xFF);
  LCD_CtrlLinesWrite(LCD_NCS_GPIO_PORT, LCD_NCS_PIN, Bit_SET);
}
...
{
  FT_SPI_WriteRead(RGB_Code >> 8);
  FT_SPI_WriteRead(RGB_Code & 0xFF);
}
